                ...
                <form id="general_settings_form">
                    ...
                    <div class="form-group">
                        ...
                        <div class="button-group">
                            <button type="button" class="btn" id="flip-display-btn" onclick="flipDisplay()" style="background: var(--primary-color);">
                                <span id="flip-btn-text">Flip Display</span>
                            </button>
                        </div>
                        <small class="form-text">Flip the display orientation (requires reboot)</small>
                    </div>
                </form>
            </div>
        </div>
    ...
    <script>
      ...
      function getSettings() {
        ...
        function loadGeneralSettings() {
            return getSettings()
                .then(settings => {
                    ...
                    // Set flip display button state
                    const flipBtnText = document.getElementById('flip-btn-text');
                    if (flipBtnText && settings.flipped !== undefined) {
        ...
        function flipDisplay() {
            // Show confirmation dialog
            if (confirm('Are you sure you want to flip the display orientation? The device will reboot after this change.')) {
                showLoading('Flipping display orientation...');
                
                // Send flip request
                fetch('/&FLIP_DISPLAY')
                    .then(response => {
                        if (response.ok) {
                            showStatus('Display orientation flipped. Device is rebooting...', 'success');
                            // The device will reboot automatically after the request
                        } else {
                            showStatus('Failed to flip display orientation.', 'error');
                            hideLoading();
                        }
                    })
                    .catch(error => {
                        console.error('Error flipping display:', error);
                        showStatus('Error flipping display orientation.', 'error');
                        hideLoading();
                    });
            }
        }
